...
import { getActiveRace, f1Races } from '@/shared/lib/f1Calendar';
...
export const LiveRaceCenter: React.FC = () => {
  const [session, setSession] = useState({
    ...
    status: 'FP1 LIVE',
    timeLeft: '18:42',
    weather: { temp: 18, trackTemp: 26, humidity: 55, chanceOfRain: 15 },
    fastestLap: '1:13.442 (PIA)'
  });

  useEffect(() => {
    const active = getActiveRace(new Date());
    const currentRace = active || f1Races.find(r => r.round === 7) || f1Races[0];
    
    setSession(prev => ({
      ...prev,
      gp: currentRace.name.toUpperCase(),
      track: currentRace.track,
      weather: {
        temp: currentRace.airTemp,
        trackTemp: currentRace.trackTemp,
        humidity: currentRace.humidity,
        chanceOfRain: currentRace.chanceOfRain
      },
      fastestLap: currentRace.fastestLap
    }));
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px', position: 'relative', zIndex: 1 }}>
        ...
        <div
          onClick={() => {
            const active = getActiveRace(new Date());
            const currentRace = active || f1Races.find(r => r.round === 7) || f1Races[0];
            window.open(currentRace.streamUrl, '_blank');
          }}
          style={{ background: 'rgba(255,24,1,0.1)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,24,1,0.2)', cursor: 'pointer', transition: 'all 0.2s' }}
          onMouseOver={(e) => {
            e.currentTarget.style.background = 'rgba(255,24,1,0.15)';
            e.currentTarget.style.borderColor = 'rgba(255,24,1,0.4)';
          }}
          onMouseOut={(e) => {
            e.currentTarget.style.background = 'rgba(255,24,1,0.1)';
            e.currentTarget.style.borderColor = 'rgba(255,24,1,0.2)';
          }}
          title="Watch live stream feed on F1 TV"
        >
      ...
      <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', fontWeight: 700, textTransform: 'uppercase' }}>FIA Node: <span style={{ color: '#fff' }}>{session.gp.substring(0, 3)}-01-SECURE</span></div>
      </div>
